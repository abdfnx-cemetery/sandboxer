  ...
  pipe(
    ...
    chain(
      ...
      (QuickJS) => {
        ...
        vm.setProp(rbHandle, "response", responseObjHandle.right);
        responseObjHandle.right.dispose();

        vm.setProp(rbHandle, "expect", expectFnHandle);
        expectFnHandle.dispose();

        vm.setProp(rbHandle, "test", testFuncHandle);
        testFuncHandle.dispose();

        vm.setProp(vm.global, "rb", rbHandle);
        rbHandle.dispose();
